  ...
  return (
    <div className="flex min-w-0 flex-1 items-center justify-between gap-3">
      <nav className="flex min-w-0 items-center gap-2 text-sm" aria-label="Lecturer portal breadcrumb">
        <Link href="/lecturer-portal" className="inline-flex items-center gap-2 rounded-md text-slate-600 outline-none transition hover:text-emerald-700 focus-visible:ring-2 focus-visible:ring-emerald-700 focus-visible:ring-offset-2">
          <span className="flex h-8 w-8 items-center justify-center overflow-hidden rounded-lg border border-emerald-100 bg-white p-0.5 shadow-sm">
        ...
        {pathname !== '/lecturer-portal' && (
          <>
            ...
            <span className="inline-flex min-w-0 items-center gap-2 font-semibold text-slate-950">
              <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-md bg-emerald-50 text-[10px] font-bold text-emerald-800">
                {currentPage.code}
              </span>
      ...
      <div className="hidden shrink-0 items-center gap-3 md:flex">
        ...
        <span className="flex h-9 w-9 items-center justify-center rounded-full border border-emerald-200 bg-emerald-50 text-xs font-black text-emerald-800 shadow-sm">
          {currentPage.code}
        </span>
      </div>
    </div>
