  ...
  return (
    <main className="min-h-screen bg-[#f6f8fb] px-4 py-5 text-slate-950 sm:px-6 lg:px-8">
      <div className="mx-auto grid min-h-[calc(100vh-2.5rem)] w-full max-w-7xl overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl shadow-slate-200/80 lg:grid-cols-[1.02fr_0.98fr]">
        <section className="relative hidden overflow-hidden bg-[#071f3f] px-10 py-10 text-white lg:flex lg:flex-col lg:justify-between xl:px-12">
          <div className="absolute inset-0 bg-[linear-gradient(135deg,_rgba(255,255,255,0.12),_transparent_42%),radial-gradient(circle_at_82%_12%,_rgba(201,151,0,0.18),_transparent_30%),linear-gradient(180deg,_#0b2d5b_0%,_#06172f_100%)]" />
          <div className="relative">
            <div className="flex items-center gap-5">
              <div className="flex h-24 w-24 shrink-0 items-center justify-center overflow-hidden rounded-2xl border border-white/35 bg-white p-3 shadow-xl shadow-slate-950/25">
                <img src="/gctu-logo.jpg" alt="GCTU logo" className="h-full w-full object-contain" />
              </div>
              <div>
                <p className="text-xs font-bold uppercase tracking-[0.22em] text-yellow-100">Ghana Communication Technology University</p>
            ...
            <div className="mt-12 max-w-2xl">
              <p className="text-xs font-bold uppercase tracking-[0.2em] text-yellow-100">Official promotion management platform</p>
              ...
              <p className="mt-5 max-w-xl text-sm leading-6 text-blue-50/85">
          ...
          <div className="relative grid grid-cols-2 gap-3">
            {trustSignals.map((item) => (
              <div key={item.label} className="rounded-xl border border-white/15 bg-white/[0.07] px-4 py-3 shadow-sm shadow-slate-950/10">
                <p className="text-[11px] font-bold uppercase tracking-[0.16em] text-yellow-100/80">{item.value}</p>
        ...
        <section className="flex items-center justify-center bg-white px-5 py-8 sm:px-8 lg:px-12">
          <div className="w-full max-w-md">
            <div className="mb-7 rounded-2xl border border-blue-100 bg-[#f4f7fc] p-4 shadow-sm lg:hidden">
