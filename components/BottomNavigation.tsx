    ...
    <nav className="fixed inset-x-0 bottom-0 z-30 border-t border-brand-border bg-white/95 px-2 py-1.5 shadow-[0_-8px_24px_rgba(17,24,39,0.08)] backdrop-blur lg:hidden" aria-label="Primary mobile navigation">
      <div className="grid min-w-0 grid-cols-5 gap-1">
        {lecturerItems.map((item) => {
          ...
          return (
            <Link key={item.href} href={item.href} className={cn('flex min-h-14 min-w-0 flex-col items-center justify-center gap-1 rounded-lg px-1 text-[11px] font-semibold text-brand-muted transition', active && 'bg-brand-primarySoft text-brand-primary')}>
